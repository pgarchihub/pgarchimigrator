...
// the shadow-table / WAL flow; see engines/postgresql/shadowflow for that.
//
...
// defaultDropColumnRollbackWindow mirrors engines/postgresql/shadowflow's FR-08a
...
// soft-drop state — engines/postgresql/reaper's finalizeDropColumn identifies
...
// engines/postgresql/reaper can perform that same sanity check without duplicating
// the literal string.
...
// engines/postgresql/reaper can recognize and clean up an orphaned one (left behind
...
// engines/postgresql/reaper independently sweeps up any orphan this leaves behind
...
// the FR-08a pattern engines/postgresql/shadowflow already uses for the
...
//
// Once the window expires without a Rollback call, engines/postgresql/reaper's
...
func (f *DDLFlow) executeAddIndex(ctx context.Context, job *state.Job) error {
	...
	// comment (engines/postgresql/progress) for why: the check itself already
...
//     engines/postgresql/reaper's finalizeDropColumn has actually dropped the
//     column, there is nothing left to roll back.
//
// SAFETY GUARD (ADD_COLUMN path only): unlike engines/postgresql/shadowflow (which
...
//
// SAFETY GUARD: unlike engines/postgresql/shadowflow (which has an explicit,
...
func (f *DDLFlow) rollbackDropColumn(ctx context.Context, job *state.Job) error {
	if job.Phase != state.PhaseRollbackWindow {
		return fmt.Errorf("ddlflow: cannot roll back DROP_COLUMN job in phase %s (expected ROLLBACK_WINDOW) — "+
			"either the column was never soft-dropped, or the window already closed and engines/postgresql/reaper "+
	...
	if job.RollbackDeadline != nil && time.Now().UTC().After(*job.RollbackDeadline) {
		return fmt.Errorf("ddlflow: the rollback window for this DROP_COLUMN expired at %s; "+
			"the column has likely already been finalized (permanently dropped) by engines/postgresql/reaper",
...
// engines/postgresql/reaper.quoteIdent; moving both to a shared internal/dbutil
