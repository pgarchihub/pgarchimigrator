...
type Report struct {
	...
	// see engines/postgresql/preview's buildAlterTypePreview for the identical
...
// actually transitions through — see engines/postgresql/ddlflow.executeDirectAddColumn
...
func pipelineFor(strategyName, operation string) []state.Phase {
	...
	case "EXPAND_BACKFILL":
		...
		// trigger+backfill path (see engines/postgresql/ddlflow's
...
func Compute(job *state.Job) *Report {
	...
	// FAILED/ABORTED (see engines/postgresql/ddlflow.fail and engines/postgresql/reaper.ScanOnce)
...
func (r *Report) Render() string {
	...
	case r.Terminal && r.CurrentPhase == state.PhaseAborted:
		...
		// engines/postgresql/reaper.ScanOnce (orphan cleanup) and a user-triggered
...
// engines/postgresql/ddlflow (see its own quoteIdent), so the statements shown here
...
func describeOperation(job *state.Job) (summary string, statements []string) {
	...
	case "DROP_COLUMN":
		...
		if job.DeprecatedColumnName == "" {
			...
			// engines/postgresql/preview.Generate; reusing bare `nil` here
			// reintroduced the identical class of bug in a different
			// function. See ListSchemas' doc comment in engines/postgresql/catalog
