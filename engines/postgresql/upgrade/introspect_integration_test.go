...
//	go test ./engines/postgresql/upgrade/... -tags=integration -v -run TestIntrospect -timeout 60s
//	go test ./engines/postgresql/upgrade/... -tags=integration -v -run TestCreateTableOnTarget -timeout 60s
...
// see that file's own comments for why engines/postgresql/upgrade needs a
// genuinely SEPARATE second instance, unlike every other integration
// test in this repo (engines/postgresql/shadowflow's own), which syncs within one
// instance.
const (
