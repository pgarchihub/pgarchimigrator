...
// engines/postgresql/shadowflow already uses for a single table, generalized
...
const (
	...
	// the DDL to recreate them on the target — engines/postgresql/catalog.ListColumns,
	// generalized across a whole database rather than one table, the
	// same introspection engines/postgresql/shadowflow.createPartitionedShadowTable
	...
	// generalization of engines/postgresql/shadowflow's own initial sync + delta
