...
func (r *Reaper) ScanOnce(ctx context.Context) (*ScanResult, error) {
	...
	for _, job := range staleJobs {
		...
		// EXPAND_BACKFILL jobs (engines/postgresql/ddlflow's ADD_COLUMN/RENAME_COLUMN
		// path) create a temporary partial index for the duration of the
		// backfill loop (see engines/postgresql/ddlflow's createBackfillIndex doc
...
//   - "DROP_COLUMN" (engines/postgresql/ddlflow's two-phase drop): finalizeDropColumn
//     performs the actual, irreversible ALTER TABLE DROP COLUMN.
//   - everything else (engines/postgresql/shadowflow's shadow-table strategy): the
...
// phase of engines/postgresql/ddlflow's two-phase drop (see
...
// index (see engines/postgresql/ddlflow's createBackfillIndex doc comment) left
// behind on the given table — normally cleaned up by a `defer` in
// engines/postgresql/ddlflow's executeExpandBackfill/executeRenameColumn, reachable
...
// mirror engines/postgresql/ddlflow's identically-named helper — see that
...
//
// Duplicated here rather than exported from engines/postgresql/ddlflow: this
// project's established convention (see quoteIdent's several independent
// copies across engines/postgresql/ddlflow, engines/postgresql/shadowflow, engines/postgresql/preview,
// engines/postgresql/progress) is a small, self-contained per-package helper over
