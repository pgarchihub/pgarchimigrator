...
// mirrors engines/postgresql/progress.Report.Render's style for a consistent feel
...
func Generate(ctx context.Context, pool *pgxpool.Pool, tableStats orchestrator.TableStatsFetcher, req orchestrator.MigrationRequest) (*Report, error) {
	...
	report := &Report{
		...
		// engines/postgresql/catalog; missed here until now.
		Statements: []string{},
		Warnings:   []string{},
		Notes:      []string{},
	}
...
// across engines/postgresql/ddlflow, engines/postgresql/reaper, engines/postgresql/shadowflow; moving
