...
// job. Exported so other packages — specifically engines/postgresql/reaper, which
...
// mirrors engines/postgresql/reaper's older, already-established orphan-cleanup
...
// because engines/postgresql/reaper's already-established contract reads those two
...
// transition is a natural extension of engines/postgresql/reaper's sweep (TODO).
func (f *ShadowFlow) Execute(ctx context.Context, job *state.Job) error {
	...
	// in engines/postgresql/ddlflow's Job.RowsProcessed (see that package's
...
func (f *ShadowFlow) prepare(ctx context.Context, job *state.Job, names resourceNames, castColumn, castType string, deps *DependentObjects) (pglogrepl.LSN, error) {
	...
	if castColumn != "" {
		...
		// engines/postgresql/ddlflow's identical checks for its own DDL-building
		// call sites.
...
// engines/postgresql/ddlflow for why every DDL-inlined value needs its own
...
// column definitions (via engines/postgresql/catalog.ListColumns — the same
...
func (f *ShadowFlow) runInitialSync(ctx context.Context, job *state.Job, names resourceNames, pkCols []string, castColumn, castType string) error {
	cfg := InitialSyncConfig{
		...
		// this exists at all — mirrors engines/postgresql/ddlflow's backfillLoop's
...
// engines/postgresql/ddlflow.DDLFlow.fail for consistency).
...
// job FAILED: engines/postgresql/reaper's ListStale query deliberately excludes
...
func (f *ShadowFlow) failAndCleanup(ctx context.Context, job *state.Job, names resourceNames, deps *DependentObjects, cause error) error {
	...
	// found via a shadow table engines/postgresql/reaper could never clean up,
