...
> (Zero-Downtime Migration) and `internal/upgrade` (Database Migration)
...
- **`SHADOW_TABLE`** (`internal/shadowflow`) — for incompatible type
  ...
  replication.
- **`internal/upgrade`** (Database Migration) — moves a whole database
  via logical replication **between two separate instances**.

`internal/upgrade`'s `CreatePublication` function **always** uses
...
> (both `SHADOW_TABLE` and `internal/upgrade`) needs a superuser
