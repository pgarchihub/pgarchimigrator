...
- **Packages with only stdlib dependencies** (`internal/strategy`,
  `internal/engines/postgresql/progress`, `internal/entitlement`, `internal/ecosystem` —
...
| `internal/engines/postgresql/progress` | 41 | Progress computation, phase display |
...
| `internal/engines/postgresql/upgrade` (excluding SQLite) | 19 | `ConnectionProvider`, `Introspect` helpers |
...
implementation), `internal/engines/postgresql/ddlflow`, `internal/engines/postgresql/shadowflow`,
`internal/orchestrator`, `internal/api`, `internal/engines/postgresql/db`,
...
3. **Cross-language verification of SQLite logic** — for example,
   `internal/engines/postgresql/upgrade`'s schema-migration logic (`PRAGMA table_info` →
...
|---|---|
| `internal/auth` | 26 |
| `internal/engines/postgresql/ddlflow` | 51 |
| `internal/orchestrator` | 17 |
| `internal/state` | 12 |
| `internal/engines/postgresql/reaper` | 12 |
| `internal/api` | 120 |
...
|---|---|---|
| `internal/engines/postgresql/shadowflow` | 30 | Shadow-table sync within a single instance |
| `internal/engines/postgresql/upgrade` | 9 | Full flow **across two separate** PostgreSQL instances (`pg-logical` ↔ `pg-upgrade-target`) |

`internal/engines/postgresql/upgrade`'s integration tests are especially important, since
...
- **Schema/table checkbox picker (Priority 3)** — was not yet
  ...
  feature.
- **`internal/engines/postgresql/upgrade`'s ecosystem OAuth2 endpoint** (`POST
