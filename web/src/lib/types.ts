// Mirrors internal/api's JSON shapes exactly — see internal/engines/postgresql/progress.Report,
// internal/engines/postgresql/preview.Report, and internal/api/server.go's request/response
...
// Mirrors internal/engines/postgresql/progress.Analytics/StrategyStats — computed entirely
...
export interface StageView {
  Phase: Phase;
  // Mirrors internal/engines/postgresql/progress's StageStatus constants exactly —
...
// Mirrors internal/engines/postgresql/progress.Report. CreatedAt/UpdatedAt are Go time.Time
...
// Mirrors internal/engines/postgresql/progress.ResourceStatus — a LIVE, directly-verified
...
// Mirrors internal/engines/postgresql/preview.Report.
...
// Mirrors internal/engines/postgresql/catalog.ColumnInfo.
...
// Mirrors internal/engines/postgresql/catalog.SampleRowsResult.
export interface SampleRowsResult {
  Columns: string[];
  Rows: string[][];
}

// Mirrors internal/engines/postgresql/db.ConnectionInfo — deliberately has no password
...
export interface ConnectionInfo {
  ...
  // Mirrors internal/engines/postgresql/db.VersionSupportStatus's exact string values —
  // "" (unknown), "below_minimum", "supported", "newer_than_tested".
  // Computed server-side (see internal/engines/postgresql/db.ClassifyVersion) so this
...
// Mirrors internal/engines/postgresql/upgrade.Phase's exact string values — deliberately a
...
// Mirrors internal/engines/postgresql/upgrade.Job — note this has NO JSON tags on the Go
...
export interface UpgradeJob {
  ...
  // UpgradeJob was first written here — see internal/engines/postgresql/upgrade.Job's own
...
// Mirrors internal/engines/postgresql/upgrade.Table — same "no JSON tags, PascalCase"
...
// Mirrors internal/engines/postgresql/upgrade.TableRef's own json tags (schema/table).
...
export interface StartUpgradeRequest {
  ...
  targetDsn: string;
  schemas?: string[];
  // Optional — see internal/engines/postgresql/upgrade.Job.SourceReplicationRef's own doc
  ...
  // schemas (see internal/engines/postgresql/upgrade.Job.Tables' own doc comment): the
