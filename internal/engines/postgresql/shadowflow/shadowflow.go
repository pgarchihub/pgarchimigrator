...
// job. Exported so other packages — specifically internal/engines/postgresql/reaper, which
...
// mirrors internal/engines/postgresql/reaper's older, already-established orphan-cleanup
...
// because internal/engines/postgresql/reaper's already-established contract reads those two
...
// transition is a natural extension of internal/engines/postgresql/reaper's sweep (TODO).
func (f *ShadowFlow) Execute(ctx context.Context, job *state.Job) error {
	...
	// in internal/engines/postgresql/ddlflow's Job.RowsProcessed (see that package's
...
func (f *ShadowFlow) prepare(ctx context.Context, job *state.Job, names resourceNames, castColumn, castType string, deps *DependentObjects) (pglogrepl.LSN, error) {
	...
	if castColumn != "" {
		...
		// internal/engines/postgresql/ddlflow's identical checks for its own DDL-building
		// call sites.
...
// internal/engines/postgresql/ddlflow for why every DDL-inlined value needs its own
...
// column definitions (via internal/engines/postgresql/catalog.ListColumns — the same
...
func (f *ShadowFlow) runInitialSync(ctx context.Context, job *state.Job, names resourceNames, pkCols []string, castColumn, castType string) error {
	cfg := InitialSyncConfig{
		...
		// this exists at all — mirrors internal/engines/postgresql/ddlflow's backfillLoop's
...
// internal/engines/postgresql/ddlflow.DDLFlow.fail for consistency).
...
// job FAILED: internal/engines/postgresql/reaper's ListStale query deliberately excludes
...
func (f *ShadowFlow) failAndCleanup(ctx context.Context, job *state.Job, names resourceNames, deps *DependentObjects, cause error) error {
	...
	// found via a shadow table internal/engines/postgresql/reaper could never clean up,
