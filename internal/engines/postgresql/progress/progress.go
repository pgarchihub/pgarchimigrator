...
type Report struct {
	...
	// see internal/engines/postgresql/preview's buildAlterTypePreview for the identical
...
// actually transitions through — see internal/engines/postgresql/ddlflow.executeDirectAddColumn
...
func pipelineFor(strategyName, operation string) []state.Phase {
	...
	case "EXPAND_BACKFILL":
		...
		// trigger+backfill path (see internal/engines/postgresql/ddlflow's
...
func Compute(job *state.Job) *Report {
	...
	// FAILED/ABORTED (see internal/engines/postgresql/ddlflow.fail and internal/engines/postgresql/reaper.ScanOnce)
...
func (r *Report) Render() string {
	...
	case r.Terminal && r.CurrentPhase == state.PhaseAborted:
		...
		// internal/engines/postgresql/reaper.ScanOnce (orphan cleanup) and a user-triggered
...
// internal/engines/postgresql/ddlflow (see its own quoteIdent), so the statements shown here
...
func describeOperation(job *state.Job) (summary string, statements []string) {
	...
	case "DROP_COLUMN":
		...
		if job.DeprecatedColumnName == "" {
			...
			// internal/engines/postgresql/preview.Generate; reusing bare `nil` here
			// reintroduced the identical class of bug in a different
			// function. See ListSchemas' doc comment in internal/engines/postgresql/catalog
