...
// the shadow-table / WAL flow; see internal/engines/postgresql/shadowflow for that.
//
...
// defaultDropColumnRollbackWindow mirrors internal/engines/postgresql/shadowflow's FR-08a
...
// soft-drop state — internal/engines/postgresql/reaper's finalizeDropColumn identifies
...
// internal/engines/postgresql/reaper can perform that same sanity check without duplicating
// the literal string.
...
// internal/engines/postgresql/reaper can recognize and clean up an orphaned one (left behind
...
// internal/engines/postgresql/reaper independently sweeps up any orphan this leaves behind
...
// the FR-08a pattern internal/engines/postgresql/shadowflow already uses for the
...
func (f *DDLFlow) executeAddIndex(ctx context.Context, job *state.Job) error {
	...
	// comment (internal/engines/postgresql/progress) for why: the check itself already
...
//     internal/engines/postgresql/reaper's finalizeDropColumn has actually dropped the
//     column, there is nothing left to roll back.
//
// SAFETY GUARD (ADD_COLUMN path only): unlike internal/engines/postgresql/shadowflow (which
...
//
// SAFETY GUARD: unlike internal/engines/postgresql/shadowflow (which has an explicit,
...
func (f *DDLFlow) rollbackDropColumn(ctx context.Context, job *state.Job) error {
	if job.Phase != state.PhaseRollbackWindow {
		return fmt.Errorf("ddlflow: cannot roll back DROP_COLUMN job in phase %s (expected ROLLBACK_WINDOW) — "+
			"either the column was never soft-dropped, or the window already closed and internal/engines/postgresql/reaper "+
	...
	if job.RollbackDeadline != nil && time.Now().UTC().After(*job.RollbackDeadline) {
		return fmt.Errorf("ddlflow: the rollback window for this DROP_COLUMN expired at %s; "+
			"the column has likely already been finalized (permanently dropped) by internal/engines/postgresql/reaper",
...
// internal/engines/postgresql/reaper.quoteIdent; moving both to a shared internal/dbutil
