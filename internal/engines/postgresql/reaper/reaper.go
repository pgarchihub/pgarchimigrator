...
func (r *Reaper) ScanOnce(ctx context.Context) (*ScanResult, error) {
	...
	for _, job := range staleJobs {
		...
		// EXPAND_BACKFILL jobs (internal/engines/postgresql/ddlflow's ADD_COLUMN/RENAME_COLUMN
		// path) create a temporary partial index for the duration of the
		// backfill loop (see internal/engines/postgresql/ddlflow's createBackfillIndex doc
...
//   - "DROP_COLUMN" (internal/engines/postgresql/ddlflow's two-phase drop): finalizeDropColumn
//     performs the actual, irreversible ALTER TABLE DROP COLUMN.
//   - everything else (internal/engines/postgresql/shadowflow's shadow-table strategy): the
...
// phase of internal/engines/postgresql/ddlflow's two-phase drop (see
...
// index (see internal/engines/postgresql/ddlflow's createBackfillIndex doc comment) left
// behind on the given table — normally cleaned up by a `defer` in
// internal/engines/postgresql/ddlflow's executeExpandBackfill/executeRenameColumn, reachable
...
// mirror internal/engines/postgresql/ddlflow's identically-named helper — see that
...
//
// Duplicated here rather than exported from internal/engines/postgresql/ddlflow: this
// project's established convention (see quoteIdent's several independent
// copies across internal/engines/postgresql/ddlflow, internal/engines/postgresql/shadowflow, internal/engines/postgresql/preview,
// internal/engines/postgresql/progress) is a small, self-contained per-package helper over
