...
func newTestServer(t *testing.T, store *fakeStore, flow *fakeFlow) (*Server, *testUsers) {
	...
	srv := NewServer(orch, store, nil, authService, nil, nil, nil, nil, false, nil, db.ConnectionInfo{}) // nil Reaper: sweep endpoint tested separately; nil pool: preview endpoint needs a real Postgres, tested in internal/engines/postgresql/preview instead
...
// through SHADOW_TABLE — a combination internal/engines/postgresql/shadowflow has no logic
...
// internal/engines/postgresql/preview.Generate is ever called (that function needs a real
// PostgreSQL connection, which this pure-unit test suite doesn't have;
// see internal/engines/postgresql/preview's own integration tests for the substantive
...
// pair and is covered by internal/engines/postgresql/upgrade's own
