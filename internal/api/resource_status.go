...
// orphaned shadow table behind that internal/engines/postgresql/reaper could never clean
...
// dependency error — see internal/engines/postgresql/shadowflow's RevertSequenceOwnership
...
func (s *Server) backfillResourceStatus(ctx context.Context, job *state.Job) []progress.ResourceStatus {
	...
	// backfillIndexName's own doc comment in internal/engines/postgresql/ddlflow — it's
	...
	// internal/engines/postgresql/reaper's own cleanup already uses to find one.
