...
//	go test ./internal/upgrade/... -tags=integration -v -run TestIntrospect -timeout 60s
//	go test ./internal/upgrade/... -tags=integration -v -run TestCreateTableOnTarget -timeout 60s
...
// see that file's own comments for why internal/upgrade needs a
// genuinely SEPARATE second instance, unlike every other integration
// test in this repo (internal/shadowflow's own), which syncs within one
// instance.
const (
